...
class AuthSignupHome(Website):
    ...
    def _signup_with_values(self, token, values):
        ...
        sales_type = params.get('sales_type')
        #END
        ...
        if is_seller and is_seller == 'on':
            values.update({
                ...
                # Added by KMS
                'seller_subscription_id': int(sub_plans),
...
#                     raise UserError(_("Please Check term and condition."))
                ...
                if  qcontext.get("mp_terms_conditions") != None:
                    try:
                        ...
                        pwd_len = len(pwd)
                        if pwd_len  < 8:
                            qcontext["error"] = _("Your password needs to contain at least 8 characters.")
...
class website_marketplace_dashboard(http.Controller):
    ...
    def submit_as_seller(self, **post):
        ...
        # Added by KMS
        sub_plans = post.get('sub_plans', False)
        sales_type = post.get('sales_type', False)
        # END
        ...
        if country_id and url_handler:
            ...
            draft_seller_group_id = request.env['ir.model.data'].get_object_reference(
                'odoo_marketplace', 'marketplace_draft_seller_group')[1]
            groups_obj = request.env["res.groups"].browse(
                draft_seller_group_id)
            if groups_obj:
                for group_obj in groups_obj:
                    group_obj.sudo().write(
                        {"users": [(4, current_user.id, 0)]})
...
class MarketplaceSellerProfile(http.Controller):
    ...
    def seller(self, seller_id=None, seller_url_handler=None, page=0, category=None, search='', ppg=False, **post):
        ...
        if seller and not seller.website_published:
            ...
            if seller and not group_exist and user_obj.url_handler != seller.url_handler:
                return request.render("http_routing.403")
        if not ppg:
        ...
        products = env['product.template'].sudo().search([('sale_ok', '=', True), ('status', '=', "approved"),
                  ("website_published", "=", True), ("marketplace_seller_id", "=", seller.id)],
                                                     limit=ppg, offset=pager['offset'], order='website_sequence desc')
    ...
    def _get_seller_search_domain(self, search):
        ...
        if search:
            for srch in search.split(" "):
                ...
                # Added By KMS
                domain += ['|', ('name', 'ilike', srch), ('seller_shop_id.name', 'ilike', srch)]
        return domain
    ...
    def load_mp_all_seller(self, page=0, search='', ppg=False, **post):
        ...
        if values.get("pager").get('page_end').get('num') < page:
            ...
            view = request.render("theme_xtremo.wk_lazy_list_sellers", values)
            return view
        else:
...
class MarketplaceSellerShop(http.Controller):
    ...
    def seller_shop_recently_product(self, shop_id, page=0, category=None, search='', ppg=False, **post):
        ...
        recently_product_obj = request.env['product.template'].search([('sale_ok', '=', True), ('status', '=', "approved"), ("website_published", "=", True), (
            "marketplace_seller_id", "=", shop_obj.seller_id.id)], order='create_date desc, website_sequence desc', limit=recently_product)
        product_count = len(recently_product_obj.ids)
        pager = request.website.pager(
            url=url, total=product_count, page=page, step=20, scope=7, url_args=post)
        product_ids = request.env['product.template'].search([("id", "in", recently_product_obj.ids)], limit=ppg, offset=pager[
