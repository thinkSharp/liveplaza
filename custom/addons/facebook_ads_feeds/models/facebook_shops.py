...
class FacebookMerchantShop(models.Model):
    ...
    @api.depends('website_id','enable_token','feed_token')
    def _get_feed_url(self):
        for obj in self:
            if obj.shop_url:
                url = str(obj.shop_url)+"/shop/"+str(obj.id)+"/content"
                if obj.feeds_security == "automatic" and  obj.enable_token and obj.feed_token:
    ...
    def get_shop_url(self):
        for obj in self:
            if obj.website_id:
                obj.shop_url = obj.website_id.get_base_url()
            else:
                obj.shop_url = False
    ...
    shop_url=fields.Char(name="Shop URL",help="Write your domain name of your website", compute="get_shop_url")
    ...
    def _get_one_product_mapping(self,product,field_mapping_lines):
        ...
        if self.multi_images:
            ...
            for image in product.product_template_image_ids:
                url = d.get('BASE_URL')+"/web/image/product.image/"+str(image.id)+"/image_1024/600x600"
                if self.update_image:
                    url += "?mlsec=%s"%(int(time() * 1000))
                additiona_images.append(url)
    ...
    def _default_designed_function(self,key,value,d):
        if(key == 'price'):
            ...
            image_url = d.get('BASE_URL')+"/web/image/product.product/"+d.get('ID')+"/image_1024/600x600"
            if self.update_image:
                image_url += "?mlsec=%s"%(int(time() * 1000))
            return image_url
        elif (key == 'inventory'):
