...
class VoucherVoucher(models.Model):
    ...
    def get_default_values(self, field=False):
        ...
        vals['max_expiry_date'] = irDefault.get('res.config.settings',
                                                'wk_coupon_max_expiry_date') or datetime.now().date()
        ...
        vals['use_minumum_cart_value'] = irDefault.get('res.config.settings',
                                                       'wk_coupon_minumum_cart_value_usage') or False
        ...
        for rec in self:
            if rec.marketplace_seller_id:
                products = self.env['product.template'].search(
                    [('status', '=', 'approved'), ('active', '=', True), ('website_published', '=', True),
					 ('sale_ok', '=', True), ('marketplace_seller_id', '=', rec.marketplace_seller_id.id)])
                rec.seller_product_ids = products
            else:
                products = self.env['product.template'].search(
                    [('status', '=', 'approved'), ('active', '=', True), ('sale_ok', '=', True), ('website_published', '=', True)])
                rec.seller_product_ids = products
        ...
        if self.customer_type == 'special_customer':
            ...
            if res:
                wizard_id = self.env['wizard.message'].create(
                    {'text': 'Email has been sent successfully to the selected customer..'}).id
    ...
    def create(self, vals):
        ...
        if vals['voucher_value'] < self.get_default_values('wk_coupon_min_amount'):
            raise ValidationError(
                ('You can`t create voucher below this minimum amount (%s) !!!') % self.get_default_values(
                    'wk_coupon_min_amount'))
        if vals['voucher_value'] > self.get_default_values('wk_coupon_max_amount'):
            raise ValidationError(
                ('You can`t create voucher greater than this maximum amount (%s) !!!') % self.get_default_values(
                    'wk_coupon_max_amount'))
        ...
        if vals.get('expiry_date'):
            diff = (datetime.strptime(str(vals.get('expiry_date')), '%Y-%m-%d').date() - datetime.strptime(
                vals.get('issue_date'), '%Y-%m-%d').date()).days
            if int(diff) >= 0:
                diff += 1
            vals['validity'] = diff
        ...
        if vals.get('expiry_date') and datetime.now().date() > datetime.strptime(str(vals['expiry_date']),
                                                                                 '%Y-%m-%d').date():
            raise ValidationError(('Expiry date have been passed already. Please select a correct expiry date..'))
        if vals.get('issue_date') and datetime.now().date() > datetime.strptime(str(vals.get('issue_date')),
                                                                                '%Y-%m-%d').date():
            raise ValidationError((
                                      'Voucher Applicable date is not correct. Either you can make voucher applicable from today or from a future date.'))
        ...
        for voucher in self:
            ...
            if vals.get('validity', False) or vals.get('issue_date', False):
                ...
                if vals.get('validity') > 0:
                    exp_date = (datetime.strptime(str(vals['issue_date']), '%Y-%m-%d').date() + timedelta(
                        days=vals.get('validity')))
                    vals['expiry_date'] = exp_date
                    if max_expiry_date and vals['expiry_date'] < datetime.strptime(str(max_expiry_date),
                                                                                   '%Y-%m-%d').date():
                        vals['expiry_date'] = max_expiry_date
            if vals.get('validity') == 0:
                raise ValidationError(('Validity can`t be 0. Choose -1 for unlimited or greater than 0 !!!'))
            if vals.get('voucher_value') and vals.get('voucher_value') < self.get_default_values(
                    'wk_coupon_min_amount'):
                raise ValidationError(
                    ('You can`t create voucher below this minimum amount (%s) !!!') % self.get_default_values(
                        'wk_coupon_min_amount'))
            ...
            if vals.get('expiry_date'):
                if vals.get('issue_date'):
                    diff = (datetime.strptime(str(vals['expiry_date']), '%Y-%m-%d').date() - datetime.strptime(
                        str(vals.get('issue_date')), '%Y-%m-%d').date()).days
                else:
                    diff = (datetime.strptime(str(vals['expiry_date']), '%Y-%m-%d').date() - datetime.strptime(
                        str(voucher.read(['issue_date'])[0]['issue_date']), '%Y-%m-%d').date()).days
                if int(diff) >= 0:
                    diff += 1
                vals['validity'] = diff
            if vals.get('voucher_value') and vals.get('voucher_value') > self.get_default_values(
                    'wk_coupon_max_amount'):
                raise ValidationError((
                                          'You can`tstrptime create voucher greater than this maximum amount (%s) !!!') % self.get_default_values(
                    'wk_coupon_max_amount'))
            ...
            history_obj = self.env['voucher.history'].sudo().search(
                [('voucher_id', '=', self.id), ('transaction_type', '=', 'credit')])
            if history_obj:
    ...
    def _get_voucher_obj_by_code(self, secret_code, refrence):
        ...
        if refrence and refrence == 'ecommerce':
            self_obj = self.search([('voucher_code', '=', secret_code), ('active', 'in', [True, False]),
                                    ('voucher_usage', 'in', ['both', 'ecommerce'])])
        if refrence and refrence == 'pos':
            self_obj = self.search([('voucher_code', '=', secret_code), ('active', 'in', [True, False]),
                                    ('voucher_usage', 'in', ['both', 'pos'])])
        return self_obj
    ...
    def _validate_n_get_value(self, secret_code, wk_order_total, product_ids, refrence=False, partner_id=False, all_products_list=False):
        ...
        if self_obj.use_minumum_cart_value and wk_order_total and wk_order_total < self_obj.minimum_cart_amount:
            result['type'] = _('ERROR')
            result['message'] = _(
                'In order to use this voucher your total order should be equal or greater than %s') % self_obj.minimum_cart_amount
            return result
        ...
        if self_obj.voucher_val_type == 'percent':
            ...
            unit = request.website.currency_id.symbol
        result['message'] = _('Validated successfully. Using this voucher you can make discount of %s %s.') % (
        voucher_value, unit)
        return result
    ...
    @api.model
    def redeem_voucher_create_histoy(self, voucher_name=False, voucher_id=False, amount=False, order_id=False,
                                     order_line_id=False, refrence=False, parter_id=False):
        result = {}
        status = False
        hist_values = {}
        history_obj = False
            ...
            if refrence != 'pos' and voucher_obj.customer_type == 'general':
                if voucher_obj.total_available > 0:
                    voucher_obj.sudo().write({'total_available': voucher_obj.total_available - 1,
                                              'date_of_last_usage': datetime.now().date()})
